...
def load_history_prompt(f_name: Union[pathlib.Path, str] = None) -> ChatPromptTemplate:
    if isinstance(f_name, str) and f_name:
        f_name = pathlib.Path(f_name)
    if f_name and f_name.is_file():
        template = json.load(f_name.open("r"))
        ...
        logger.warning(
            f"No history prompt provided. Using default history prompt from {__name__}"
        )
        template = {
            "system_template": "You are an helpful AI assistant that is given a conversation between a user and "
            "wandbot - An AI assistant for Weight & Biases. \nYour task is to rephrase the "
            "follow-up input to be a standalone input to wandbot.\n - if the follow-up input is "
            "not related to the chat history or weights & biases, copy the follow-up input "
            "verbatim for wandbot to answer.\n - if the follow-up input is a question related to "
            "the chat history and weights & biases, summarize the chat and rephrase the follow-up "
            "input to be a standalone question to wandbot.\n - if the follow-up input is a "
            "statement or feedback related to the chat history and weights & biases, rephrase the "
            "follow-up input to be a standalone statement or feedback to wandbot.\n\nChat "
            "History:\n{chat_history}\nGiven the chat history and no prior knowledge, rephrase the "
            "follow-up input to be a standalone input to wandbot.\n",
...
def load_eval_prompt(f_name: Union[pathlib.Path, str] = None) -> ChatPromptTemplate:
    ...
    if f_name and f_name.is_file():
        ...
        template = {
            "system_template": "You are an evaluator for the W&B chatbot. You are given a question, the chatbot's "
            "answer, and the original answer, and are asked to score the chatbot's answer as "
            "either CORRECT or INCORRECT. Note that sometimes, the original answer is not the best "
            "answer, and sometimes the chatbot's answer is not the best answer. You are evaluating "
