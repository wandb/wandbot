...
from wandbot.evaluation.eval.utils import safe_parse_eval_response
...
class PureCorrectnessEvaluator:
    ...
    async def safe_parse_eval_response(
        ...
        print(f"Raw LLM response: {eval_response}", flush=True)
        try:
    ...
    async def evaluate(
        ...
        **kwargs: Any,
        ...
        eval_response = await self._get_completion(self.system_template, user_prompt)
