...
class LiteLLMEmbeddings(Embeddings):
    def __init__(
        ...
        elif hf_api_base is not None: # Hugging Face
            self.args["model"] = model
            self.args["api_base"] = hf_api_base
        else: # Rest
            self.args["model"] = model
...
class HuggingFaceEmbedding(Embeddings):
    """For Nvidia GPU enabled devices. Not optimized. Tested on A100.
    """
    def __init__(
        self,
        ...
        self.task = 'Given a Weights and Biases user query, retrieve relevant contexts that answer the query'
    ...
    def embed_query(self, text: str) -> List[float]:
        """Embed query text."""
        text = self.get_detailed_instruct(self.task, text)
